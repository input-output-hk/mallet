...
const testnets = {
  ...
  iele: {
    rpc: 'https://staging.iele.dev-mantis.iohkdev.io:8546',
    faucet: 'https://staging.iele.dev-mantis.iohkdev.io:8099/faucet',
    compiler: 'https://5c177bzo9e.execute-api.us-east-1.amazonaws.com/prod'
  }
}
...
class Iele {
  ...
  compileSol(source) {
    ...
    if (codeStarted) {
      ...
      const bytecode = this.compileIele(code).result;
  ...
  compileIele(source) {
    ...
    if (request.status == 200) {
      ...
      try {
        ...
        throw 'Compiler error: failed to parse JSON response';
      }
  ...
  compile(path) {
    ...
    if (path.endsWith('.iele')) {
      return this.compileIele(contents);
    } else if (path.endsWith('.sol')) {
      return this.compileSol(contents);
    } else {
